...
import { Plus, Edit, Trash2, Eye, ChevronLeft, ChevronRight, Tag, FileText, Calendar, ArrowLeft, Power, PowerOff } from 'lucide-vue-next';
...
const toggleStatus = (category: PostCategory) => {
  router.patch(`/post-categories/${category}/toggle-status`, {}, {
    preserveState: true,
    preserveScroll: true,
    onSuccess: () => {
    ...
    <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
      ...
      <div class="bg-card rounded-lg p-6 shadow-sm border border-border">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-4">
          <div class="flex-1">
            <h1 class="text-xl sm:text-2xl font-bold mb-2 text-foreground">Categorías de Posts</h1>
            <p class="text-muted-foreground text-sm sm:text-base">Gestiona las categorías para organizar tus publicaciones</p>
          </div>
          <button 
        ...
        <div class="flex gap-2 sm:gap-4 flex-wrap">
          ...
          <div class="bg-blue-50 dark:bg-blue-900/20 px-3 sm:px-4 py-2 sm:py-3 rounded-md flex-1 sm:flex-none">
            <span class="font-semibold text-blue-700 dark:text-blue-300 text-xs sm:text-sm">Con Posts: {{ categories.data.filter(c => c.posts_count > 0).length }}</span>
          </div>
          <div class="bg-purple-50 dark:bg-purple-900/20 px-3 sm:px-4 py-2 sm:py-3 rounded-md flex-1 sm:flex-none">
            <span class="font-semibold text-purple-700 dark:text-purple-300 text-xs sm:text-sm">Total Posts: {{ categories.data.reduce((sum, c) => sum + c.posts_count, 0) }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="bg-card rounded-lg overflow-hidden shadow-sm border border-border">
        ...
        <div v-else class="p-8 text-center">
          <div class="flex flex-col items-center gap-4">
            ...
            <div>
              <h3 class="text-lg font-semibold text-foreground mb-2">No hay categorías de posts</h3>
              <p class="text-muted-foreground mb-4">Comienza creando tu primera categoría para organizar tus publicaciones.</p>
              <button 
                @click="openCreateModal"
                class="inline-flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground hover:bg-primary/90 rounded-md transition-colors font-medium"
              >

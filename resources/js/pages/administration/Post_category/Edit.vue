...
import { update } from '@/routes/post-categories'
...
watch(() => props.category, (newCategory) => {
    if (newCategory) {
        ...
        editForm.is_active = newCategory.is_active
    }
}, { immediate: true })
...
const submitEdit = () => {
    ...
    editForm.put(update(props.category.id).url, {
        onSuccess: () => {
            emit('updated')
            emit('close')
        }
        ...
        <div class="bg-card rounded-lg shadow-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div class="p-6">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-lg font-semibold text-foreground">Editar Categoría</h2>
                ...
                <form @submit.prevent="submitEdit" class="space-y-4">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        />
                        <label for="edit-is_active" class="text-sm font-medium text-foreground">Categoría activa</label>
                        <div v-if="editForm.errors.is_active" class="text-sm text-red-600 mt-1">{{ editForm.errors.is_active }}</div>
                    </div>

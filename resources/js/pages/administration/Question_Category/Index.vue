...
const selectedCategory = ref<EvaluationCategory | null>(null);
...
const openEditModal = (category: EvaluationCategory) => {
  selectedCategory.value = category;
  showEditModal.value = true;
};

const openDeleteModal = (category: EvaluationCategory) => {
  selectedCategory.value = category;
...
const deleteCategory = (category: EvaluationCategory) => {
...
const goToPage = (page: number) => {
  if (page >= 1 && page <= (props.categories?.last_page || 1)) {
    router.visit('/question-categories', {
      data: { page },
      preserveState: true,
      preserveScroll: true,
    });
...
<template>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
      ...
      <div class="bg-card rounded-lg p-6 shadow-sm border border-border">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-4">
          <div class="flex-1">
            <h1 class="text-xl sm:text-2xl font-bold mb-2 text-foreground">Categorías de Evaluación</h1>
            <p class="text-muted-foreground text-sm sm:text-base">Gestiona las categorías para las preguntas de evaluación</p>
          </div>
          <button 
        ...
        <div class="flex gap-2 sm:gap-4 flex-wrap">
          ...
          <div class="bg-green-50 dark:bg-green-900/20 px-3 sm:px-4 py-2 sm:py-3 rounded-md flex-1 sm:flex-none">
            <span class="font-semibold text-green-700 dark:text-green-300 text-xs sm:text-sm">Activas: {{ categories.data.filter(c => c.is_active).length }}</span>
          </div>
          <div class="bg-blue-50 dark:bg-blue-900/20 px-3 sm:px-4 py-2 sm:py-3 rounded-md flex-1 sm:flex-none">
            <span class="font-semibold text-blue-700 dark:text-blue-300 text-xs sm:text-sm">Con Preguntas: {{ categories.data.filter(c => c.questions_count > 0).length }}</span>
          </div>
          <div class="bg-purple-50 dark:bg-purple-900/20 px-3 sm:px-4 py-2 sm:py-3 rounded-md flex-1 sm:flex-none">
            <span class="font-semibold text-purple-700 dark:text-purple-300 text-xs sm:text-sm">Total Preguntas: {{ categories.data.reduce((sum, c) => sum + c.questions_count, 0) }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="bg-card rounded-lg overflow-hidden shadow-sm border border-border">
        ...
        <div v-else class="p-8 text-center">
          <div class="flex flex-col items-center gap-4">
            ...
            <div>
              <h3 class="text-lg font-semibold text-foreground mb-2">No hay categorías de evaluación</h3>
              <p class="text-muted-foreground mb-4">Comienza creando tu primera categoría para organizar las preguntas de evaluación.</p>
              <button 
                @click="openCreateModal"
                class="inline-flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground hover:bg-primary/90 rounded-md transition-colors font-medium"
              >
    ...
    <DeleteModal 
      v-if="showDeleteModal && selectedCategory"
      :category="selectedCategory"
      @close="closeModals"

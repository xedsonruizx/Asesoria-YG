...
interface Category {
    ...
    description?: string;
    slug: string;
    questions_count?: number; // Agregar el conteo de preguntas
}
...
// Función para verificar si una pregunta debe mostrarse (MOVER ANTES de visibleQuestionsByCategory)
const shouldShowQuestion = (question: Question): boolean => {
    ...
    // Manejar tanto 'expected_value' como 'value' para compatibilidad
    const expectedValue = question.show_condition.expected_value || question.show_condition.value;
    
    // Si no hay respuesta en la pregunta padre, no mostrar la pregunta dependiente
    if (conditionAnswer === undefined || conditionAnswer === null || conditionAnswer === '') {
        return false;
    }
    
    // Debug log para verificar las condiciones
    // console.log('🔍 Evaluando condición:', {
    //     questionId: question.id,
    //     parentAnswer: conditionAnswer,
    //     expectedValue: expectedValue,
    //     operator: question.show_condition.operator
    // });
    
    switch (question.show_condition.operator) {
        case 'equals':
            // Para radio buttons que ahora usan texto en lugar de boolean
            if (typeof expectedValue === 'string' && typeof conditionAnswer === 'string') {
                return conditionAnswer === expectedValue;
            }
            // Para compatibilidad con valores boolean antiguos
            if (typeof expectedValue === 'boolean') {
                if (expectedValue === true) {
                    return conditionAnswer === 'Sí' || conditionAnswer === true;
                }
                if (expectedValue === false) {
                    return conditionAnswer === 'No' || conditionAnswer === false;
                }
            }
            return conditionAnswer === expectedValue;
        case 'not_equals':
...
const visibleQuestionsByCategory = computed(() => {
    // Debug: Log de categorías recibidas desde el backend
    // console.log('🏷️ Categorías recibidas desde backend:', props.categories);
    // console.log('❓ Preguntas recibidas desde backend:', props.questions);
    
    // Filtrar categorías que tienen al menos 1 pregunta (usando questions_count del backend)
    const categoriesWithQuestions = props.categories
        .filter(category => {
            // Si tenemos questions_count del backend, usarlo
            if (category.questions_count !== undefined) {
                return category.questions_count > 0;
            }
            // Fallback: contar preguntas manualmente
        ...
        .map(category => {
            ...
            //     questions: categoryQuestions.map(q => ({ id: q.id, text: q.question_text }))
            // });
            
            // Filtrar preguntas visibles basándose en show_condition usando la función shouldShowQuestion
            const visibleQuestions = categoryQuestions.filter(question => {
                const shouldShow = shouldShowQuestion(question);
                // if (!shouldShow) {
                //     console.log(`🚫 Pregunta oculta (ID: ${question.id}):`, {
                //         question: question.question_text,
                //         condition: question.show_condition,
                //         parentAnswer: question.show_condition ? answers.value[question.show_condition.parent_question_id] : 'N/A'
                //     });
                // }
                return shouldShow;
            });
            
            // console.log(`👁️ Preguntas visibles en "${category.name}":`, visibleQuestions.length);
    ...
    // Solo devolver categorías que tienen preguntas visibles (doble filtro para mayor seguridad)
    const finalCategories = categoriesWithQuestions.filter(category => category.questions.length > 0);
    
    // console.log('📋 Categorías finales con preguntas visibles:', finalCategories.map(c => ({
    //     name: c.name,
    //     id: c.id,
...
// Inicialización
onMounted(() => {
    // console.log('🚀 Componente Evaluation montado');
    // console.log('📊 Props completas:', props);
    // console.log('🏷️ Categorías:', props.categories);
    // console.log('❓ Preguntas:', props.questions);
    // console.log('💾 Respuestas iniciales:', props.answers);
    
    // Verificar si hay categorías sin preguntas
    props.categories.forEach(category => {
        const questionsInCategory = props.questions.filter(q => q.category_id === category.id);
        console.log(`📂 Categoría "${category.name}" tiene ${questionsInCategory.length} preguntas`);
        
        if (questionsInCategory.length === 0) {
            console.warn(`⚠️ La categoría "${category.name}" no tiene preguntas asignadas`);
        }
    });
    
    // Verificar si hay preguntas sin categoría válida
    props.questions.forEach(question => {
        const hasValidCategory = props.categories.some(cat => cat.id === question.category_id);
        if (!hasValidCategory) {
            console.warn(`⚠️ La pregunta "${question.question_text}" tiene category_id ${question.category_id} que no existe en las categorías`);
        }
    });
...
// Función debug para resetear el formulario
const debugResetForm = () => {
    if (confirm('¿Estás seguro de que quieres resetear todas las respuestas? Esta acción no se puede deshacer.')) {
        // Limpiar respuestas
        answers.value = {};
        ...
        showNotification('success', 'Formulario reseteado correctamente');
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <Card class="mb-8 p-6">
                <div class="text-center">
                    ...
                    <p class="text-lg font-medium" :class="getProgressColor(overallPercentage)">
                        Progreso: {{ overallPercentage }}%
                    </p>
                </div>
            </Card>
            ...
            <Button 
                @click="debugResetForm" 
                variant="destructive" 
                size="sm"
                class="bg-red-600 hover:bg-red-700 text-white shadow-lg"
            >
                🐛 Debug Reset
            </Button>
                </div>
